...
class ModelNet40(VisionDataset):
    ...
    classnames = ["airplane,aeroplane,plane",
    ...
    def __init__(self, root, num_cam, split='train', per_cls_instances=0, dropout=0.0):
        ...
        for cls in self.classnames:
            for fname in sorted(glob.glob(f'{root}/{cls}/{split}/*.png')):
                ...
                attr = fname.split('.')[0].split('_')
                id, cam = attr[0], int(attr[1])
                e, a, r = int(attr[2]), int(attr[3]), int(attr[4])
                is_planar = 'planar' in fname
                is_short = 'short' in fname
                if cam >self.num_cam:
                    continue
                ...
                self.meta[cam - 1].append({'e': e, 'a': a, 'r': r,
                                        'planar': is_planar,
                                        'short': is_short})
    ...
    def __getitem__(self, idx, visualize=False):
        ...
        return imgs, tgt, keep_cams, metas
