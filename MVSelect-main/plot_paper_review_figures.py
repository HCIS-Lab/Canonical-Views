...
C=['#0072B2','#E69F00','#009E73','#CC79A7','#666666','#56B4E9','#D55E00','#332288']
...
def curve(ax,g,x,y,label,color,scale=1,ls='-',band=True):
 ...
 ax.plot(xx,v,label=label,color=color,lw=1.6,ls=ls)
...
for i,v in enumerate(asset['views']):
 example=np.asarray(Image.open(AS/v['file']).convert('RGB')); x0,y0,x1,y1=asset['display_viewport']['pixel_bounds'];example=example[y0:y1,x0:x1];sub=a.inset_axes([i/3+.015,.12,.29,.65]);sub.imshow(example);sub.axis('off')
 a.text(i/3+.16,.97,v['name'],ha='center',va='top',fontsize=9,transform=a.transAxes)
 a.text(i/3+.16,-.03,v['description'].replace(' extends','\nextends').replace(' points','\npoints').replace(' viewed','\nviewed'),ha='center',va='top',fontsize=8,transform=a.transAxes)
...
for line in c.lines:
 if line.get_linestyle()=='-':line.set_marker('o');line.set_markersize(3)
c.set_xticks([1,20,40,60,80,100]);c.set_ylabel('Selected views (%)');c.set_ylim(0,65);c.legend(loc='upper left',ncol=2,frameon=False,fontsize=8);c.text(.98,.95,'Dotted: candidate shares',transform=c.transAxes,ha='right',va='top',fontsize=8)
e=fig.add_subplot(gs[3,0],label='d');tag(e,'d');e.set_title('View families',loc='left')
for m,l,col,p in [('expanded_family','Expanded family',C[0],19.5285),('foreshortened_family','Foreshortened family',C[2],13.6294),('Remainder','Remainder',C[4],66.8421)]:
...
for i,(m,l) in enumerate([('ellipse_aspect_ratio','Ellipse aspect ratio'),('bilateral_symmetry','Bilateral symmetry'),('edge_entropy','Edge-orientation entropy')]):
 ax=axes[i];ax.set_label(string.ascii_lowercase[i]);tag(ax,string.ascii_lowercase[i]);curve(ax,ds,'epoch','selected_'+m,'Mean of selected views',C[0]);ax.axhline(ds['baseline_'+m].iloc[0],color='#555',ls='--',label='Mean of all candidate views');ax.set_ylabel(l);ax.set_xlabel('Training epoch' if i==2 else '');ax.set_xticks([1,20,40,60,80,100]);ax.tick_params(axis='x',labelbottom=True)
...
# Main restriction and freezing: identical viewing regimes in separate panels.
for name,conds in [('figure_3',['free','expanded_family','foreshortened_family','foreshortened_family_remainder','remainder']),('figure_4',['free','freeze_10','freeze_20','freeze_30'])]:
 current=name;fig,axes=plt.subplots(1,3,figsize=(9.1,3.3),sharey=True)
 for j,(metric,title) in enumerate([('accuracy','Policy-selected inputs (6)'),('random_accuracy','Random inputs (5)'),('all_views_accuracy','All candidate inputs (114)')]):
  ax=axes[j];ax.set_label(string.ascii_lowercase[j]);tag(ax,string.ascii_lowercase[j]);ax.set_title(title);ax.set_ylim(0,100)
  for i,cid in enumerate(conds):curve(ax,training[training.condition_id==cid],'training_epoch',metric,CL[cid],C[i])
 axes[0].set_ylabel('Object-category accuracy (%)');h,l=axes[0].get_legend_handles_labels();fig.legend(h,l,loc='lower center',ncol=3 if len(conds)==5 else 4,frameon=False,bbox_to_anchor=(.52,-.12));fig.subplots_adjust(wspace=.12,bottom=.19);save(fig,current)
# Replay paired runs, no unmatched original-model line.
current='figure_5';fig,axes=plt.subplots(1,2,figsize=(9.2,5),sharey=True)
for j,(metric,title) in enumerate([('final_accuracy_pct','Object-category accuracy (%)'),('final_prediction_margin','Top-1 minus top-2 logit margin')]):
...
for j,(metric,title) in enumerate([('delta_replace_correct_logit','Correct-class logit'),('delta_replace_prediction_margin','Top-1 minus top-2 margin')]):
 ...
 for i,fam in enumerate(F):
  v=rep[(rep.epoch==100)&(rep.view_family==fam)][metric];ax.scatter(v,np.full(len(v),i)+np.linspace(-.08,.08,len(v)),s=18,color=C[i]);ax.errorbar(v.mean(),i,xerr=v.std(ddof=1),fmt='D',mfc='white',mec='black',ecolor='black',ms=4,capsize=3)
 ax.invert_yaxis()
...
# ED3 single-control trajectories and principal-planar upper bound.
current='extended_data_3';fig,axes=plt.subplots(2,2,figsize=(8.2,6.5));primary=['single_primary','single_high_lr','single_high_select_lr']
for i,cid in enumerate(primary):
 ax=axes.flat[i];tag(ax,string.ascii_lowercase[i]);ax.set_label(string.ascii_lowercase[i]);ax.set_title(['Single image: primary learning rates','Single image: higher recognition rate','Single image: higher rates for both'][i]);g=training[training.condition_id==cid]
 for m,l,col,p in [('expanded_family','Expanded family',C[0],19.5285),('foreshortened_family','Foreshortened family',C[2],13.6294),('Remainder','Remainder',C[4],66.8421)]:curve(ax,g,'training_epoch',m,l,col,100);ax.axhline(p,color=col,ls=':',lw=.8)
 ax.set_ylim(0,100);ax.set_ylabel('Selected views (%)')
ax=axes.flat[3];tag(ax,'d');ax.set_title('Planar views');ax.set_label('d');curve(ax,geom[geom.metric=='exact_planar'],'epoch','selected_fraction','Multiview: direct count',C[0],100)
u=pd.read_csv(D/'single_planar_upper_bound.csv');curve(ax,u[u.condition_id=='single_primary'],'training_epoch','principal_planar_upper_bound_pct','Single image: upper bound',C[1],ls='--');ax.axhline(100*6/114,color='#333',ls=':',label='Candidate share');ax.set_ylabel('Selected views (%)');ax.legend(frameon=False,fontsize=7)
...
for name,conds in [('extended_data_4',['free','expanded_family','foreshortened_family','foreshortened_family_remainder','remainder']),('extended_data_5',['free','freeze_10','freeze_20','freeze_30','freeze_40','freeze_50'])]:
 ...
 for i,cid in enumerate(conds):
  ax=axes.flat[i];ax.set_label(string.ascii_lowercase[i]);tag(ax,string.ascii_lowercase[i]);ax.set_title(CL[cid]);ax.set_ylim(0,100);ax.tick_params(axis='x',labelbottom=True)
...
for j,budget in enumerate([5,6]):
 ...
 for i,(run,r) in enumerate(g.groupby('run')):
  v=r.set_index('sampling').loc[['random','policy'],'accuracy_pct'];ax.plot([0,1],v,'o-',color=C[i],lw=1,ms=4,label=f'Model {i+1}')
 ax.set_xticks([0,1],['Random','Policy selected']);ax.set_xlim(-.3,1.3);ax.grid(axis='y',alpha=.2)
axes[0].set_ylabel('Object-category accuracy (%)');axes[1].legend(frameon=False,fontsize=7);fig.tight_layout();save(fig,current)
...
for j,obj in enumerate(['correct_logit','prediction_margin']):
 ax=axes[j];tag(ax,string.ascii_lowercase[j]);z=co[co.objective==obj].pivot(index='feature',columns='epoch',values='correlation').reindex(['ellipse_aspect_ratio','bilateral_symmetry','edge_entropy']);ax.imshow(z,vmin=-.2,vmax=.2,cmap='RdBu_r',aspect='auto');ax.set_yticks(range(3),['Ellipse aspect ratio','Bilateral symmetry','Edge-orientation entropy']);ax.set_xticks(range(10),range(10,101,10));ax.set_title(['Correct-class logit','Top-1 minus top-2 margin'][j]);ax.set_xlabel('Source-selection epoch')
 for y in range(3):
...
for j,(df,metric,label) in enumerate([(clip,'accuracy','CLIP accuracy (%)'),(vg,'mean','VGGT object-mask confidence')]):
 ...
 for i,fam in enumerate(F):
  v=df[df.view_type==fam][metric];assert len(v)==5,(fam,df.view_type.unique());ax.bar(i,v.mean(),width=.62,color=C[i],alpha=.35);ax.scatter(i+np.linspace(-.13,.13,5),v,s=17,color=C[i]);ax.text(i,v.max()+(.9 if j==0 else .23),f'{v.mean():.2f}',ha='center',fontsize=8)
...
for row,(metric,title) in enumerate([('accuracy_pct','Object-category accuracy (%)'),('prediction_margin','Top-1 minus top-2 margin')]):
