...
class VGGTModel(MultiViewModel):
    ...
    def extract_single_view_confidence(self, img):
        """Run VGGT on a single image. Equivalent to monocular depth precision
        — there is no other view to cross-attend to.

        Returns:
            conf: Tensor (1, 1, H, W) on CPU.
        """
        images = self.preprocess([img]).to(self.device)
        with torch.no_grad():
            with torch.amp.autocast("cuda", dtype=self.dtype, enabled="cuda" in self.device):
                predictions = self.model(images)
        conf = predictions["depth_conf"].detach().cpu()  # (1, 1, H, W)
        del predictions, images
        clear_gpu_memory()
        return conf
