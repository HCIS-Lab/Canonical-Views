...
class Pi3Model(MultiViewModel):
    ...
    def extract_single_view_confidence(self, img):
        """Run Pi3 on a single image. Returns (1, 1, H, W) conf map on CPU."""
        imgs_tensor = self.preprocess([img]).to(self.device)
        with torch.no_grad():
            with torch.amp.autocast("cuda", dtype=self.dtype, enabled="cuda" in self.device):
                results = self.model(imgs_tensor[None])
        conf_maps = torch.sigmoid(results["conf"][0, ..., 0]).detach().cpu()  # (1, H, W)
        conf_maps = conf_maps.unsqueeze(0)  # (1, 1, H, W)
        del results, imgs_tensor
        clear_gpu_memory()
        return conf_maps
